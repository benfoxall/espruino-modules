...
var unlock = function() { // unlock memory
  poke32(KEYR, 0x45670123);
  poke32(KEYR, 0xCDEF89AB);
};

var read = function(addr) { // read a 32 bit word
  if (addr&3) return false; // wrong offset
  return peek32(addr);
};

var write = function(addr, val) { // write a 32 bit word
  if (addr&3) return false; // wrong offset
  wait();
...
var erase = function(addr) { // erase a page
...
export { unlock, read, write, erase };export default STM32F1Flash;
